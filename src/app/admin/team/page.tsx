      ...
      <div className="max-w-xl">
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            {editing.photo ? (
              <div className="relative w-32 h-32 rounded-full bg-muted overflow-hidden mb-2">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={editing.photo} alt="" className="w-full h-full object-cover" />
                <button onClick={() => setEditing((p) => ({ ...p, photo: "" }))} className="absolute top-1 right-1 rounded-full bg-background border border-border p-1 hover:bg-muted">
                  <X className="h-3 w-3" />
                </button>
              </div>
            ) : (
      ...
      <div className="space-y-2">
        {members.map((m) => (
          <div key={m.id} className="flex items-center gap-4 border border-border px-5 py-4 bg-background hover:bg-muted/30 transition-colors">
            ...
            <RowActions
              actions={[
                { label: m.published ? "Hide" : "Show", icon: m.published ? EyeOff : Eye, onClick: () => togglePublish(m) },
                { label: "Edit", icon: Pencil, onClick: () => openEdit(m.id) },
                { label: "Delete", icon: Trash2, onClick: () => setConfirmDeleteId(m.id), disabled: deleting === m.id, destructive: true },
              ]}
            />
          </div>
        ))}
