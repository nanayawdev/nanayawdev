...
export default function AdminCaseStudiesPage() {
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={editing.client ?? ""} onChange={(e) => setEditing((p) => ({ ...p, client: e.target.value }))}
                className="w-full rounded-lg border border-border bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-foreground" placeholder="Thomisia Travel" />
            </div>
            <div>
              <label className="mb-1.5 block text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Category</label>
              <Select value={editing.category ?? "Web Development"} onValueChange={(value) => setEditing((p) => ({ ...p, category: value }))}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={editing.year ?? ""} onChange={(e) => setEditing((p) => ({ ...p, year: e.target.value }))}
                className="w-full rounded-lg border border-border bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-foreground" placeholder="2024" />
            </div>
            <div>
              ...
              <input value={editing.client_url ?? ""} onChange={(e) => setEditing((p) => ({ ...p, client_url: e.target.value }))}
                className="w-full rounded-lg border border-border bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-foreground" placeholder="https://..." />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button type="button" onClick={addService} className="rounded-full border border-border px-4 py-2 text-[0.65rem] font-semibold uppercase tracking-[0.18em] hover:bg-muted transition-colors">Add</button>
            </div>
              ...
              {stats.slice(0, 3).map((stat, i) => (
                <div key={i} className="grid grid-cols-2 gap-3">
                  <input value={stat.value} onChange={(e) => updateStat(i, "value", e.target.value)}
                    className="rounded-lg border border-border bg-background px-4 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-foreground" placeholder={`Value ${i + 1}, e.g. 3×`} />
                  <input value={stat.label} onChange={(e) => updateStat(i, "label", e.target.value)}
                    className="rounded-lg border border-border bg-background px-4 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-foreground" placeholder={`Label ${i + 1}, e.g. Increase in bookings`} />
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {studies.map((cs) => (
          <div key={cs.id} className="flex items-center gap-4 rounded-lg border border-border px-5 py-4 bg-background hover:bg-muted/30 transition-colors">
            {cs.cover_image && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={cs.cover_image} alt="" className="h-12 w-20 rounded-lg object-cover shrink-0 border border-border" />
            )}
            ...
            <span className={`text-[0.6rem] font-semibold uppercase tracking-[0.18em] rounded-lg px-2.5 py-1 border ${cs.published ? "border-green-500 text-green-600" : "border-border text-muted-foreground"}`}>
