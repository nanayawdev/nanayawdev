        ...
        {testimonials.map((t) => (
          <div key={t.id} className="flex items-center gap-4 border border-border px-5 py-4 bg-background hover:bg-muted/30 transition-colors">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <button onClick={() => togglePublish(t)} title={t.published ? "Hide" : "Show"} className="text-muted-foreground hover:text-foreground transition-colors">
                {t.published ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              </button>
              <button onClick={() => openEdit(t.id)} title="Edit" className="text-muted-foreground hover:text-foreground transition-colors">
                <Pencil className="h-4 w-4" />
              </button>
              <button onClick={() => setConfirmDeleteId(t.id)} disabled={deleting === t.id} title="Delete" className="text-muted-foreground hover:text-red-500 transition-colors disabled:opacity-40">
