        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select value={editing.category ?? "Web"} onValueChange={(value) => setEditing((p) => ({ ...p, category: value }))}>
      ...
      <div className="space-y-2">
        {projects.map((p) => (
          <div key={p.id} className="flex items-center gap-4 rounded-lg border border-border px-5 py-4 bg-background hover:bg-muted/30 transition-colors">
            ...
            <RowActions
              actions={[
                { label: p.published ? "Unpublish" : "Publish", icon: p.published ? EyeOff : Eye, onClick: () => togglePublish(p) },
                { label: "Edit", icon: Pencil, onClick: () => openEdit(p.id) },
                { label: "Delete", icon: Trash2, onClick: () => setConfirmDeleteId(p.id), disabled: deleting === p.id, destructive: true },
              ]}
            />
          </div>
        ))}
